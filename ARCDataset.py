...
class ARCTrain(Dataset):
    ...
    def __init__(self, root, imgsz=15, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        return support_x[:-1, None], support_y[:-1].reshape(-1), support_x[-1, None][None],\
            support_y[-1].reshape(-1)


        # query_x = torch.tensor(self.val_x_batch[index], dtype=torch.float32)
        # query_y = torch.tensor(self.val_y_batch[index], dtype=torch.long)
        # return support_x[:, None], support_y.reshape(-1), query_x[:, None], query_y.reshape(-1)
